...
int main(void)
{
    ...
    int ulT = 180;			//horizontal treshold
    int ulBCy = 45;		//vertical border control vertical back treshold
    int hmX = -40;			//relative x home coordinate
    int hmY = 130;			//y home coordinate
    
    //compound eye constants
    char eyeCH = 10;	//begin chase
    int eyeSA = 70;		//begin track
    int eyeSB = 20;		//end chase when in front
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if ( eyePP<d2 )
            {
                ...
                traS=cap((absl(ulX-ulZ+d1)/2+absl(ulY-hmY)+10)*3/2,100); //align to base power
                if(traS<30){
                	traS = 0; //stop if distance less than 30
                }
            ...
            else
            {
                ...
                if ( eyeDR<=180) traD=eyeDR+cap((d1*(145-eyePP))/80,95);
                else if ( eyeDR>180 ) traD=eyeDR-cap((d1*(145-eyePP))/80,95);
                else traD=eyeDR;
